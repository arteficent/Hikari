...
```bash
Serverless$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```
...
```bash
Serverless$ sam logs -n NetCodeWebAPIServerless --stack-name Serverless --tail
```
...
```bash
Serverless$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name Serverless
```
